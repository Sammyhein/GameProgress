...
import { modifyGame } from "../actions/user-gamesActions"
import { usePathname } from "next/navigation"

type ModifyGameModalProps = {
  gameId: number
  gameName: string
  onClose: () => void
...
export default function ModifyGameModal({
  gameId,
  gameName,
  onClose,
  actualProgress,
  actualPlayedTime,
  actualScale
}: ModifyGameModalProps) {
  const pathname = usePathname()
  const [progress, setProgress] = useState(actualProgress)
  const [playedTime, setPlayedTime] = useState(actualPlayedTime)
  const [scale, setScale] = useState(actualScale)
  ...
  const handleSubmit = async () => {
    setIsPending(true)
    const result = await modifyGame(gameId, progress, playedTime, scale, pathname)
    setIsPending(false)
      ...
      <article className="flex flex-col gap-6">
        ...
        <header>
          <h2 className="text-lg font-bold text-text-primary">
            Modifier <span className="text-brand-cyan">{gameName}</span>
          </h2>
        ...
        <form className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              max={10}
              value={scale ?? ""}
              onChange={(e) => setScale(e.target.value ? Number(e.target.value) : null)}
              placeholder="Ex: 8"
              className="bg-bg-elevated border border-brand-purple/20 focus:border-brand-purple focus:outline-none text-text-primary placeholder:text-text-muted rounded-xl px-4 py-3 text-sm transition-colors"
            />
          </div>
...
// import { useEffect, useRef, useState } from "react"
// import { modifyGame , type AddGameState} from "../actions/user-gamesActions"
// import { usePathname } from "next/navigation"
...
// export default function ModifyGameModal({gameId, gameName, onClose, actualProgress, actualPlayedTime, actualScale}: ModifyGameModalProps){
//     const pathname = usePathname()
//     const [progress, setProgress] = useState(actualProgress)
//     const [playedTime, setPlayedTime] = useState(actualPlayedTime)
//     const [scale, setScale] = useState(actualScale)
...
//                 <header>
//                     <h2 className="text-lg font-bold">Modifier vos infos sur {gameName}</h2>
//                 </header>
